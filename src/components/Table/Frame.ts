...
export default class Frame extends Diagram<FrameProps> {
    ...
    private static readonly TABLETITLE: string[] = ['工程标尺', '月', '日', '进度标尺', '工程周'];
    ...
    private _weekCell: any | undefined;
    ...
    init(): void {
        ...
        this.setWeekCell(this._weekCell);
        ...
        this.CANVAS.add(...this._weekCell.generate());
    ...
    private setCycleCell(cycleCell: any): void {
        ...
        cycleCell.setStartTime(this.getStartTime());
    }
    ...
    private setDayCell(dayCell: any): void {
        ...
        dayCell.setHeight(this.getHeight());
    ...
    private setWeekCell(weekCell: any): void {
        weekCell.setX(Frame.TABLELEFTWIDTH + this.getX());
        weekCell.setY(this.getY() + this.getLineHeight() * 3);
        weekCell.setLineHeight(this.getLineHeight());
        weekCell.setOffset(this.getOffset());
        weekCell.setScale(this.getScale());
        weekCell.setStartTime(this.getStartTime());
    }
    ...
    private textStyle(text: string, index: number): object {
        return {
            top: index > 3 ? (this.getHeight() - this.getY() - this.getLineHeight() * 2) + this.getLineHeight() * (index - 4) + ((this.getLineHeight() - this.getFontSize()) / 2)
            ...
            if (i !== 4) {
                this._object.push(
                    new fabric.Line(
                        [
                            this.getX(),
                            (this.getHeight() - this.getY() - this.getLineHeight() * 2) + this.getLineHeight() * (i - 1),
                            this.getWidth() - this.getX() + Frame.TABLEBORDERWIDTH,
                            (this.getHeight() - this.getY() - this.getLineHeight() * 2) + this.getLineHeight() * (i - 1)
                        ],
                        {
